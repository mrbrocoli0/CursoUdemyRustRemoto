...
fn dame_ownership<'a>(param: String) -> &'a str {
  &param // esto hace que no compile!
    //param sólo vive en el lifetime a que sólo es válido en función dame_ownership, cuando quiere
    //pasar el valor a main ya param ya no existe
}

fn dame_ref<'a, 'b>(param_a: &'a i32, param_b: &'a i32) -> &'a i32 {
  if param_a > param_b {
    param_a
    //Regresa-return param_a
  } else {
    param_b
    //Regresa-return param_b
  }
}
